...
def get_had(n, decompose=False):
    hads = []
    if is_pow2(n):
        if decompose:
            pow2 = int(math.log2(n))
            hadL_dim, hadR_dim = pow2 // 2, pow2 - pow2 // 2
            hadL, hadR = get_had_pow2(int(2 ** hadL_dim)), get_had_pow2(int(2 ** hadR_dim))
            hads.extend([hadL, hadR])
        else:
            had = get_had_pow2(n)
            hads.append(had)
    else:
        hadR, K = get_hadK(n)
        hadR = hadR / torch.tensor(n).sqrt()
        if (n // K) > 1:
            hadL = get_had_pow2(n // K, norm=False)
            hads.append(hadL)
        hads.append(hadR)
    return hads
