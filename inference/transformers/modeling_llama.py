...
ALL_LAYERNORM_LAYERS.append(inference.nn.RMSNorm)
...
class FlatQuantFP16LlamaAttention(LlamaFlashAttention2):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if isinstance(self.o_proj_trans, inference.nn.OnlineTrans) and self.o_proj_trans.trans == "matmul":
...
class FlatQuantLlamaAttention(FlatQuantFP16LlamaAttention):
    ...
    def __init__(self, options, *args, **kwargs):
        ...
        self.quantizer = inference.nn.Quantizer()
        self.q_proj = inference.nn.Linear4bit.from_float(self.q_proj)
        self.k_proj = inference.nn.Linear4bit.from_float(self.k_proj)
        self.v_proj = inference.nn.Linear4bit.from_float(self.v_proj)
        if "o_proj" in self.options.online_trans:
            self.o_proj_trans = inference.nn.OnlineTrans(self.num_heads, trans=options.trans, decompose=False)
        self.o_proj = torch.nn.Sequential(
            inference.nn.Quantizer(),
            inference.nn.Linear4bit.from_float(self.o_proj)
        )
        if "qkv_proj" in self.options.online_trans:
            if not self.options.fuseLN:
                self.inp_trans = inference.nn.OnlineTrans(self.hidden_size, trans=options.trans)
...
class FlatQuantLlamaMLP(LlamaMLP):
    def __init__(self, options, *args, **kwargs):
        ...
        self.options = options
        self.quantizer = inference.nn.Quantizer()
        self.up_proj = inference.nn.Linear4bit.from_float(self.up_proj)
        self.gate_proj = inference.nn.Linear4bit.from_float(self.gate_proj)
        if "down_proj" in self.options.online_trans:
            self.down_proj = torch.nn.Sequential(
                inference.nn.OnlineTrans(self.intermediate_size, trans=options.trans),
                inference.nn.Quantizer(),
                inference.nn.Linear4bit.from_float(self.down_proj)
            )
        else:
            self.down_proj = torch.nn.Sequential(
                inference.nn.Quantizer(),
...
class FlatQuantLlamaForCausalLM(FlatQuantFP16LlamaForCausalLM):
    def __init__(self, args, config):
        ...
        assert config._attn_implementation == "flash_attention_2"
        if args.fuseLN:
            self.norm = inference.nn.RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        for layer_idx, layer in enumerate(self.model.layers):
            layer.self_attn = FlatQuantLlamaAttention(options=args, config=config, layer_idx=layer_idx)
            if args.fuseLN:
                layer.input_layernorm = inference.nn.RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
                layer.post_attention_layernorm = inference.nn.RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
