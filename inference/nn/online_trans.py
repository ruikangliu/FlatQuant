...
class OnlineTrans(torch.nn.Module):
    def __init__(self, trans_dim, force_fp32=False, trans="had", decompose=True):
        ...
        if trans == "had":
            had_rem_dim, self.rem_dim = inference.functional.online_trans.get_hadK(trans_dim)
    ...
    def forward(self, x):
        if self.fp32_trans:
            x = x.float()
        if self.trans == "had":
            x = inference.functional.matmul_hadU_cuda(x, self.had_rem_dim, self.rem_dim)
            ...
            x = inference.functional.online_trans.kronecker_matmul(x, invs)
        return x
