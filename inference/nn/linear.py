...
class Linear4bit(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        assert type(x) == inference.PackedQuantizedTensor #Quantized input is given
        x, scales_x = x.quantized_x, x.scales_x
        #shape_handler = ShapeHandler(quantized_x)
        #quantized_x = shape_handler.flatten(quantized_x)
        x = inference.matmul(x, self.weight)
        #out = shape_handler.unflatten(
        #    inference.sym_dequant(int_result, scales_x, self.weight_scales))
        if self.bias is not None:
            return inference.sym_dequant(x, scales_x, self.weight_scales) + self.bias
        else:
            return inference.sym_dequant(x, scales_x, self.weight_scales)
    ...
    def from_float(module: torch.nn.Linear, weight_scales=None,):
        ...
        if weight_scales is not None:
            ...
            int_module.weight.copy_(inference.functional.pack_i4(int_rounded_weight.to(torch.int8)).cpu())
